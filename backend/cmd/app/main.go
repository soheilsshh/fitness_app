...
func NewServer(db *gorm.DB) *Server {
	...
	aiGenerateService := service.NewAIGenerateService(db, meService, aiRequestLogRepo, subscriptionRepo, programRepo)
	recipeService := service.NewRecipeService(recipeRepo)
	progressReportService := service.NewProgressReportService(db, progressReportRepo, subscriptionRepo)
	service.StartScheduler(context.Background(), progressReportService)
	poseBankService := service.NewPoseBankService(poseBankRepo)
	achievementService := service.NewAchievementService(db, achievementRepo)
	achievementService.SeedDefaultRules(context.Background())
	communityPostService := service.NewCommunityPostService(db, communityPostRepo)
	...
	coachSessionService := service.NewCoachSessionService(coachSessionRepo, notificationRepo, coachStudentService)
	...
	workoutHistoryService := service.NewWorkoutHistoryService(db, subscriptionRepo, servicePlanRepo, programRepo, achievementService)
