...
type ServicePlanRepository interface {
	...
	ListByCoachID(ctx context.Context, coachID uint, page, pageSize int, query, tag string) ([]models.ServicePlan, int64, error)
	FindByIDAndCoachID(ctx context.Context, id, coachID uint) (*models.ServicePlan, error)
...
func (r *servicePlanRepository) applyCoachListFilters(db *gorm.DB, query, tag string) *gorm.DB {
	if q := strings.TrimSpace(query); q != "" {
		like := "%" + q + "%"
		db = db.Where("name LIKE ? OR subtitle LIKE ? OR course_name LIKE ?", like, like, like)
	}
...
func (r *servicePlanRepository) ListByCoachID(ctx context.Context, coachID uint, page, pageSize int, query, tag string) ([]models.ServicePlan, int64, error) {
	db := r.db.WithContext(ctx).Model(&models.ServicePlan{}).Where("coach_id = ?", coachID)
	db = r.applyCoachListFilters(db, query, tag)
...
func (r *servicePlanRepository) FindByIDAndCoachID(ctx context.Context, id, coachID uint) (*models.ServicePlan, error) {
	var plan models.ServicePlan
	if err := r.db.WithContext(ctx).Where("id = ? AND coach_id = ?", id, coachID).First(&plan).Error; err != nil {
		return nil, err
	}
	return &plan, nil
}
